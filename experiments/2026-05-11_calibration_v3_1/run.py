...
Three changes from v3, motivated by Phase A's diagnostic:
    1. Per-archetype diel modifier (two amplitudes — land and water —
       sharing one phase). Phase A showed the N-sector signal is mostly
       nocturnal, but bay/estuary tidal cycles may peak at a different
       hour than land-side drain/channel sources. v3 collapsed both
       into one global multiplier.
    2. Bay archetype cap raised 0.5 → 5.0 g/s. v3's Otay River Outlet
       bay source hit the 0.5 cap; relaxing it should let NNLS absorb
       more N-wind NESTOR signal.
    3. Two candidate "northeast" sources at (32.580, -117.040) and
       (32.595, -117.040) — direct test of the missing-source
       hypothesis (SY uniquely elevated in N sector → source east/NE
       of SY).

Inputs / outputs / CLI: same shape as v3. Compare against v2 *and* v3
holdout numbers (the v2 + v3 fits are reproduced here for the
