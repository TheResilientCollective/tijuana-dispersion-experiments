...
Builds on v3.2's source field (12-cell NE grid + relaxed bay cap) and
adds a 5-source near-field arc around SAN YSIDRO (archetype
"sy_nearfield", ≥1.4 km from the receptor) covering the W/WNW/NW/E/S
directions that currently have no upwind source. Tests whether SY's
weak rank-order fit (Spearman ~0.16, the project's worst per the
2026-05-12 metric reframe) is a missing near-field / cross-border
source.

Runs v2 (no diel), v3 (single-amp diel — canonical form), and v3.1
(per-archetype diel — retired but kept for continuity) on the same
field. Reports Spearman (headline metric per the reframe), log-Pearson,
and Pearson. See RESULTS.md for the verdict (hypothesis rejected:
near-field sources absorb ~0 rate; SY is representativeness-limited).
"""
...
def build_source_field(config: dict[str, Any]) -> list[SourceSpec]:
    ...
    # v3.5 addition: SAN YSIDRO near-field arc (archetype "sy_nearfield").
    sy_candidates: list[SourceSpec] = []
    for spec in config["sources"].get("sy_nearfield_candidates", []):
        sy_candidates.append(
            SourceSpec(
                name=spec["name"],
                lat=spec["lat"],
                lon=spec["lon"],
                archetype="sy_nearfield",
                emission_rate_g_s=spec.get("seed_rate_g_s", 0.3),
                height_m=spec.get("height_m", 1.0),
            )
        )
    return NAMED_SOURCES + channel + estuary + ne_candidates + sy_candidates
...
# sy_nearfield is local urban / cross-border → thermal-ish → "land".
...
def per_receptor_metrics(
    ...
    for r_idx, rname in enumerate(receptor_names):
        ...
        p = pred[valid, r_idx]
        o = obs[valid, r_idx]
        corr = float(np.corrcoef(p, o)[0, 1]) if np.std(p) > 0 else float("nan")
        # Spearman is the headline metric per the 2026-05-12 metric reframe;
        # log-Pearson is the magnitude-aware secondary; raw Pearson kept for
        # continuity with older RESULTS files.
        spear = float(spearmanr(o, p).correlation) if np.std(p) > 0 else float("nan")
        log_pear = (
            float(np.corrcoef(np.log1p(o), np.log1p(np.clip(p, 0.0, None)))[0, 1])
            if np.std(p) > 0
            else float("nan")
        )
...
def main() -> None:
    ...
    summary: dict[str, Any] = {
        "experiment": "calibration_v3_5_sy_nearfield",
